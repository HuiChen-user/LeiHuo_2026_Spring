...
{
    [CustomEditor(typeof(HighTemperatureZoneSwitch))]
    public class HighTemperatureZoneSwitchEditor : Editor
    {
        public override void OnInspectorGUI()
        {
            DrawDefaultInspector();

            HighTemperatureZoneSwitch zoneSwitch = (HighTemperatureZoneSwitch)target;
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                {
                    Undo.RecordObject(zoneSwitch, "Enable High Temperature Zones");
                    RecordControlledZones(zoneSwitch, "Enable High Temperature Zones");
                ...
                {
                    Undo.RecordObject(zoneSwitch, "Disable High Temperature Zones");
                    RecordControlledZones(zoneSwitch, "Disable High Temperature Zones");
            ...
            EditorGUILayout.HelpBox(
                "Scene editing: drag the red range handle to resize the interaction area. Lines show which high-temperature zones this switch toggles.",
                MessageType.Info);
        }
        ...
        private void DrawRangeHandle(HighTemperatureZoneSwitch zoneSwitch)
        {
            ...
            if (zoneSwitch.UsesSphereRange())
            {
                ...
                {
                    Undo.RecordObject(zoneSwitch, "Resize High Temperature Switch Range");
        ...
        private void DrawTargetLines(HighTemperatureZoneSwitch zoneSwitch)
        {
            Handles.color = zoneSwitch.TargetLineColor;
            Vector3 from = zoneSwitch.GetInteractionWorldCenter();
            IReadOnlyList<LeiHuo.Gameplay.TemperatureField.HighTemperatureZone> zones = zoneSwitch.ControlledZones;

            for (int i = 0; i < zones.Count; i++)
            {
                LeiHuo.Gameplay.TemperatureField.HighTemperatureZone zone = zones[i];
                if (zone == null)
                {
                    continue;
                }
        ...
        private void DrawStatusLabel(HighTemperatureZoneSwitch zoneSwitch)
        {
            Vector3 center = zoneSwitch.GetInteractionWorldCenter();
        ...
        private void RecordControlledZones(HighTemperatureZoneSwitch zoneSwitch, string undoName)
        {
            IReadOnlyList<LeiHuo.Gameplay.TemperatureField.HighTemperatureZone> zones = zoneSwitch.ControlledZones;
            for (int i = 0; i < zones.Count; i++)
            {
        ...
        private void MarkControlledZonesDirty(HighTemperatureZoneSwitch zoneSwitch)
        {
            IReadOnlyList<LeiHuo.Gameplay.TemperatureField.HighTemperatureZone> zones = zoneSwitch.ControlledZones;
            for (int i = 0; i < zones.Count; i++)
            {
